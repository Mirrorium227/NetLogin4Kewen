...
| 路径 | 说明 |
| --- | --- |
| `dist_exe/` | 学生网 Windows 可执行文件，适合直接下载运行。 |
| `dist_tc/` | 教师网 Windows 可执行文件，适合直接下载运行。 |
| `spec/` | 学生网 PyInstaller 打包配置，便于复现构建。 |
| `spec_tc/` | 教师网 PyInstaller 打包配置，便于复现构建。 |
...
`dist_tc/` 中提供教师网 Windows 端 exe：

| 文件 | 说明 |
| --- | --- |
| `NL4KWTC_Win.exe` | 教师网普通版，单账号配置。 |
| `NL4KWTC_Win_auto.exe` | 教师网自动账号轮转版，多账号配置。 |
| `NL4KWTC_Win_Emulator.exe` | 教师网手动模拟版，适合只能通过网页登录的情况。 |
...
运行 `NL4KWTC_Win.py` 或 `NL4KWTC_Win.exe`。首次运行会生成 `config_tc.ini`，填写后再次运行。
...
运行 `NL4KWTC_Win_auto.py` 或 `NL4KWTC_Win_auto.exe`。首次运行会生成 `config_tc_auto.ini`，默认自带两个账号位。
...
运行 `NL4KWTC_Win_Emulator.py` 或 `NL4KWTC_Win_Emulator.exe`。首次运行会标定账号框、密码框和登录按钮位置。
